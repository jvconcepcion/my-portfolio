...
export async function getSkills(): Promise<SkillRow[]> {
  try {
    const { data, error } = await supabase
      .from('skills')
      .select('value, icon_id, count, color, url')
      .order('sort_order', { ascending: true });

    if (error || !data) {
      console.warn('[Supabase] Failed to fetch skills, using default list.');
...
export async function getExperience(): Promise<ExperienceRow[]> {
  try {
    const { data, error } = await supabase
      .from('experience')
      .select('company, stage, location, occupations')
      .order('sort_order', { ascending: true });

    if (error || !data) {
      console.warn('[Supabase] Failed to fetch experience, using default list.');
...
export async function getEducation(): Promise<EducationRow[]> {
  try {
    const { data, error } = await supabase
      .from('education')
      .select('school, course, location, geo_location')
      .order('sort_order', { ascending: true });

    if (error || !data) {
      console.warn('[Supabase] Failed to fetch education, using default list.');
...
function getDefaultSkills(): SkillRow[] {
  return [
...
function getDefaultExperience(): ExperienceRow[] {
  return [
    {
      ...
      occupations: [
        ...
        },
        {
          ...
          description: [
            ...
            "Technical Proficiency: Advanced expertise in PHP, C#, Yii, Laravel, and relational databases (MSSQL, MySQL), with deep experience in developing and integrating RESTful APIs to streamline backend processes.",
...
function getDefaultEducation(): EducationRow[] {
  return [
    {
